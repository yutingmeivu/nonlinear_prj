...
class NN:
    ...
    def newton_1d(self, t, partial, optim_method, rand_low, rand_up, K, tolerate, tolerate_g, tolerate_distance, cg_formula):
        # t: t_w, t_b
        # g: g_w, g_b
        # H: H_w, H_b
        N = self.x_train.shape[0]
        output = self.x_train
        back_error = self.loss_prime(output, self.y_train)
        H_back = self.loss_H(output, self.y_train)
    ...
    def lineSearch(self, t_w, t_b, optim_method, search_method, temp, linesearch, rand_low, rand_up, K, tolerate, k_index):
        # t initial: [0,0,...,0] list with length as same as number of layers
        n = len(self.layers)
        t_w = t_w if t_w else np.zeros(n)# store in reversed order, which is 0th is the last layer
        t_b = t_b if t_b else np.zeros(n)
        k = 0
    ...
    def conjugate_gradient(self, partial, search_method, rand_low, rand_up, K, tolerate_g, tolerate_distance, cg_formula, \
        ...
        while (np.linalg.norm(g_w1) > tolerate) & (k < K):
            ...
            if (dis <= tolerate_distance) | (step <= n_step) | ():
                ...
                pk1 = self.conjugate_gradient_pk(pk1, g_k, g_k1, 0, cg_formula, False, last)
                ...
                g_k1 = g_w1 if partial == 'w' else g_b1
                pk1 = - g_k1
                pk1 = self.conjugate_gradient_pk(pk1, g_k, g_k1, 0, cg_formula, False, 'conjugate')
                back_error1, g_w1, g_b1, H_w1, H_b1 = self.back_prop(back_error0, None, None, None, 'conjugate', search_method, True, True, rand_low, rand_up, K, tolerate, tolerate_g, tolerate_distance, cg_formula, g_w0, g_b0, H_w0, H_b0, pk1)
                k += 1
                step = 0
                last = 'steepest'
    ...
    def fit(self, x_train, y_train, epochs, eta, optim_method, search_method, temp, linesearch, rand_low, rand_up, K, tolerate, tolerate_g, tolerate_distance, cg_formula):
        ...
        for i in range(epochs):
            for j in range(N):
                ...
                for index, layer in enumerate(reversed(self.layers)):
                    back_error, g_w, g_b, H_w, H_b = layer.back_prop(back_error, H_back, eta, index, optim_method, False, False, rand_low, rand_up, K, tolerate, tolerate_g, tolerate_distance, cg_formula, g_w, g_b, H_w, H_b)
